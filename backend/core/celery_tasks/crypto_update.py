...
def _do_update_crypto_list():
    ...
    new_objects = [
        CryptoCurrency(coingecko_id=coin["id"], symbol=coin["symbol"].upper(), name=coin["name"])
        for coin in coins
        if coin["id"] not in existing_ids and is_valid_symbol(coin["symbol"].upper()) and is_valid_name(coin["name"])
    ]
    if new_objects:
